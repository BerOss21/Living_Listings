                ...
                <box v-for="(offer, index) in listing.offers" :key="index">
                    ...
                    <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-2">
                        ...
                        <div v-if="!listing.sold_at">
                            <Link :href="route('listings.offers.accept', [listing.id, offer.id])" as="button" method="put"
                                class="border rounded py-1 px-2 bg-green-500 text-white">Accept</Link>
                        </div>
                    </div>
                </box>
            </template>
            <AlertMessage v-else message="Any offer has been made for this listing"/>
        </section>
